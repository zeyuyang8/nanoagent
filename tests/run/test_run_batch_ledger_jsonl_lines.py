...
:func:`nanoagent.run.batch.run_batch` appends one slim ledger row per task to
...
:func:`nanoagent.run.batch.completed_ids` (splits on newlines, ``json.loads`` per line, collects the
...
over 3 distinct tasks via :func:`~nanoagent.run.batch.run_batch`, exactly the
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/run/test_run_batch_ledger_jsonl_lines.py -q
"""
...
from nanoagent.run.batch import completed_ids
