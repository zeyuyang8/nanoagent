...
path :func:`~nanoagent.run.batch.run_batch` kills a rollout via ``asyncio.wait_for(agent.run(...),
...
* ``test_run_batch_timeout_reconciles_traj_stop_reason`` — after the cap fires, the ``<task>.traj.json``
  (via :func:`~nanoagent.run.trajectory.load`) reads ``stop_reason == "error"`` and ``error`` starting
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/run/test_run_batch_timeout_traj_stop_reason.py -q
"""
