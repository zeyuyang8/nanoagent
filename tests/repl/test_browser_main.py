"""Offline unit tests for the two exit-code branches of :func:`nanoagent.repl.browser.main`.
...
Consumes: ``nanoagent.repl.browser`` (the ``main`` entry point under test) and
``nanoagent.run.trajectory.TRAJECTORY_SUFFIX`` (the real ``*.traj.json`` suffix). Pure — stdlib
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/repl/test_browser_main.py -x -q
"""
...
from nanoagent.repl import browser
