...
What it consumes: :func:`nanoagent.repl.app.run_and_save` plus the
:class:`~nanoagent.core.agent.Reply` / ``StopReason`` shapes (read-only), and
:func:`nanoagent.config.load_config` to build a :class:`~nanoagent.config.RunConfig`
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/repl/test_run_and_save.py -x -q
"""
...
from nanoagent.repl import app
from nanoagent.run import trajectory
from nanoagent.core.agent import Reply
from nanoagent.config import load_config, RunConfig


class _OneTurnModel:
    """Scripted stand-in for ``nanoagent.core.model.Model``: a single ``query`` turn.
