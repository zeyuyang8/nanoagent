"""Offline tests for :mod:`nanoagent.harness.tools.skill` and the prompt
:func:`~nanoagent.harness.run.build.build_prompt_and_tools` assembles from it (plus that function's
sibling :func:`~nanoagent.harness.run.build.context_text`, which folds in the project's own instructions).
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/tools/test_skill.py -x -q
"""
...
from nanoagent.harness.run.build import build_prompt_and_tools, context_text
from nanoagent.harness.config import AgentConfig
from nanoagent.harness.tools.skill import discover, Skill
