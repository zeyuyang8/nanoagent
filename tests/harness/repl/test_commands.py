"""Offline tests for :mod:`nanoagent.harness.repl.commands` — the REPL's slash-command table.
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/repl/test_commands.py -x -q
"""
...
from nanoagent.harness.repl import commands
from nanoagent.harness.repl.app import InteractiveSession, ReplOptions
...
def test_a_markdown_file_becomes_a_slash_command(tmp_path: Path) -> None:
    ...
    repl, _ = _session(["/review src/nanoagent/harness/core/agent.py"], commands=[str(template)])

    # The handler's return value IS the task: _read hands it straight back to the loop.
    assert repl._read("> ") == "Review src/nanoagent/harness/core/agent.py and report every bug."
...
def test_model_switches_the_backing_model(monkeypatch: Any) -> None:
    from nanoagent.harness.core import model as model_module
