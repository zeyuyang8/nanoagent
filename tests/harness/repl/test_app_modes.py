"""Offline unit tests for nanoagent's chat-mode REPL (:class:`nanoagent.harness.repl.app.InteractiveSession`).

What it consumes: :class:`~nanoagent.harness.repl.app.InteractiveSession`, the
:class:`~nanoagent.harness.core.agent.Reply` / ``ToolCall`` / ``StopReason`` shapes, and
:class:`nanoagent.harness.core.tool.Tool` — all imported read-only. No model server / network / GPU: the
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/repl/test_app_modes.py -x -q
"""
...
from nanoagent.harness.core.agent import Reply, StopReason, ToolCall
