...
:func:`nanoagent.harness.run.batch.run_batch` appends one slim ledger row per task to
...
:func:`nanoagent.harness.run.batch.completed_ids` (splits on newlines, ``json.loads`` per line, collects the
...
:class:`~nanoagent.harness.core.agent.ChatModel` returns a multi-line ``Reply.content`` in one turn (no tool
call → that reply is the final answer), and a tool-less :class:`~nanoagent.harness.core.agent.Agent` drives it
over 3 distinct tasks via :func:`~nanoagent.harness.run.batch.run_batch`, exactly the
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_run_batch_ledger_jsonl_lines.py -q
"""
...
from nanoagent.harness.run.batch import completed_ids
from nanoagent.harness.run import batch
...
class _ScriptedModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel`: answer multi-line prose in one turn.
