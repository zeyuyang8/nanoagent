...
:class:`nanoagent.harness.core.agent.Reply` / :class:`~nanoagent.harness.core.agent.ToolCall` and
:class:`nanoagent.harness.core.tool.Tool` to script the model and the one echo tool, and
:func:`nanoagent.harness.run.trajectory.save` / :func:`~nanoagent.harness.run.trajectory.load` for the on-disk
...
How to run it (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_trajectory_multi_turn_capture.py -x -q
"""
...
class _TwoToolsThenAnswerModel:
    """Scripted :class:`~nanoagent.harness.core.agent.ChatModel`: two DISTINCT echo tool calls on two
