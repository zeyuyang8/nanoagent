...
path :func:`~nanoagent.harness.run.batch.run_batch` kills a rollout via ``asyncio.wait_for(agent.run(...),
...
:class:`~nanoagent.harness.core.agent.ChatModel` asks for a tool call every turn; a scripted :class:`Tool`
...
* ``test_run_batch_timeout_reconciles_traj_stop_reason`` — after the cap fires, the ``<task>.traj.json``
  (via :func:`~nanoagent.harness.run.trajectory.load`) reads ``stop_reason == "error"`` and ``error`` starting
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_run_batch_timeout_traj_stop_reason.py -q
"""
...
from nanoagent.harness.core.agent import Agent, Reply, ToolCall
...
class _SlowSecondStepModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel` that requests ``slow_tool`` every turn.
