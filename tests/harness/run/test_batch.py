"""Offline tests for the batch driver's per-rollout wall-clock cap (:func:`nanoagent.harness.run.batch.run_batch`).

Everything is in-process — no model server is contacted. A scripted
:class:`~nanoagent.harness.core.agent.ChatModel` either blocks on ``asyncio.sleep`` (to overrun the cap) or
answers immediately (the fast, no-cap path); each test drives :func:`~nanoagent.harness.run.batch.run_batch`
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_batch.py -x -q
"""
...
class _ScriptedModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel`: optionally block, then answer in one turn.
