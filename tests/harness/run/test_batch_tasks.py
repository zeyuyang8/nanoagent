...
(:func:`nanoagent.harness.run.batch.load_tasks` and :func:`nanoagent.harness.run.batch.filter_tasks`).

nanoagent's batch run mode (the rollout fan-out) picks which tasks to run with two pure helpers in :mod:`nanoagent.harness.run.batch`: ``load_tasks`` reads a JSONL of tasks and
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_batch_tasks.py -x -q
"""
...
from nanoagent.harness.run.batch import filter_tasks, load_tasks
from nanoagent.harness.run.taskselect import _SHUFFLE_SEED
