"""Offline tests for :func:`nanoagent.harness.run.batch.run_batch`'s resume ledger, ``redo``, error
...
:class:`~nanoagent.harness.core.agent.ChatModel` answers in one turn, blocks on ``asyncio.sleep`` (to
overrun a wall-clock cap), or raises; each test drives :func:`~nanoagent.harness.run.batch.run_batch`
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_run_batch_resume.py -q
"""
...
class _ScriptedModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel`: answer in one turn, block, or raise.
