...
trajectory. When :func:`nanoagent.harness.run.batch.run_batch` fans a
...
:class:`~nanoagent.harness.core.agent.ChatModel` answers ``"DONE"`` in one turn (no tool call → that reply is
the final answer) and a tool-less :class:`~nanoagent.harness.core.agent.Agent` drives it, exactly the
``test_batch._ScriptedModel`` shape.

* ``test_run_batch_stamps_per_task_traj_meta`` — drive ``run_batch`` over 3 distinct tasks at
  ``concurrency=3`` (so the rollouts overlap), then :func:`~nanoagent.harness.run.trajectory.load` each
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_run_batch_traj_meta_stamped.py -q
"""
