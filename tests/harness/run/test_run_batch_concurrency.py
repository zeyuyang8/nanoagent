"""Offline test for the batch driver's concurrency bound (:func:`nanoagent.harness.run.batch.run_batch`).
...
* ``test_run_batch_concurrency_caps_in_flight`` — drive ``run_batch`` over 6 tasks with
  ``concurrency=2`` using a scripted :class:`~nanoagent.harness.core.agent.ChatModel` (the
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_run_batch_concurrency.py -q
"""
...
class _ConcurrencyProbeModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel` that measures rollout overlap.
