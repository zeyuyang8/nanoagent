"""Offline test for nanoagent's batch CLI orchestration wrapper (:func:`nanoagent.harness.run.cli._run_batch`).

``_run_batch`` is the batch fan-out CLI entry an RL trainer drives (rollout fan-out + resume UX). It loads a :class:`~nanoagent.harness.config.BatchConfig` from ``key=value`` tokens,
wires the progress ``on_start``/``on_done`` callbacks (``on_done`` builds the stop_reason tally via
:func:`~nanoagent.harness.run.progress.format_tally`), runs :func:`~nanoagent.harness.run.batch.run_batch`, prints
...
to hand back an :class:`~nanoagent.harness.core.agent.Agent` over a scripted model (so no real ``Model``/openai
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/run/test_run_batch_cli.py -q
"""
...
from nanoagent.harness.run import cli, trajectory
from nanoagent.harness.core.agent import Agent, Reply


class _ScriptedModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel`: answer ``"DONE"`` in one turn.
