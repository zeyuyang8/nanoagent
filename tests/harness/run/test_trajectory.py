...
Drives the REAL :class:`~nanoagent.harness.core.agent.Agent` loop with an in-process scripted
``ChatModel`` (mirrors :mod:`...tests.test_context`'s mock — no model server is contacted),
and pins the two properties of
:class:`~nanoagent.harness.run.trajectory.IncrementalTrajectoryWriter`:

* ``test_incremental_save_byte_identical`` — after every step its file is byte-for-byte equal
  to the one :func:`~nanoagent.harness.run.trajectory.save` writes (behavior preserved). The run has tool
...
Both tests import :class:`~nanoagent.harness.run.trajectory.IncrementalTrajectoryWriter`, so they error
out (fail) on the un-optimized code and pass only once the writer exists.

Run (from the repo root)::

    python3 -m pytest tests/harness/run/test_trajectory.py -x -q
"""
...
class _ScriptedModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel`: ``tool_steps`` ``noop`` turns then DONE.
