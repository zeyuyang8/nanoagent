...
(:meth:`nanoagent.harness.core.agent.Agent.run`'s post-run ``cleanup`` hook).

The bug
-------
:class:`nanoagent.harness.tools.code.CodeExec` holds its per-task sandbox dir in a per-asyncio-context
...
real :class:`nanoagent.harness.tools.code.CodeExec` — mirrors ``test_agent_run_resets_tools`` /
``test_agent_cost_accumulate`` (no model server, network, or GPU).

Run (from the repo root)::

    python3 -m pytest tests/harness/core/test_agent_run_cleans_up_sandbox.py -x -q
"""
...
from nanoagent.harness.tools.code import CodeExec
...
class _WriteThenAnswerModel:
    """Scripted :class:`~nanoagent.harness.core.agent.ChatModel`: emit ONE ``python`` tool call, then answer.
    ...
    run ends on :attr:`~nanoagent.harness.core.agent.StopReason.ANSWER` after exactly one tool call. Mirrors the
