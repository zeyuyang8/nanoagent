...
What it consumes: :class:`nanoagent.harness.core.agent.Agent` driven by an in-process scripted ChatModel that
emits ONE tool call (turn 1) then answers (turn 2), plus one async :class:`~nanoagent.harness.core.tool.Tool`
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/core/test_agent_step_durations_tool_step.py -x -q
"""
...
class _ToolThenAnswerModel:
    """Scripted :class:`~nanoagent.harness.core.agent.ChatModel`: turn 1 emits ONE tool call (-> a TOOL step
