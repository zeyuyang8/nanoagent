...
:class:`~nanoagent.harness.config.ModelConfig` is all-``MISSING`` (a run config must set every knob
...
The price of two schemas is that they can drift, so :meth:`nanoagent.harness.core.model.Model.from_config`
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/core/test_model_config_projection.py -x -q
"""
