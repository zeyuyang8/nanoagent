"""Pin Agent.run's cost-limit guard at the EXACT-equality boundary (:mod:`nanoagent.harness.core.agent`).
...
``cost >= self._cost_limit`` at the TOP of :meth:`~nanoagent.harness.core.agent.Agent.run`'s loop. The only
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/core/test_agent_cost_limit_boundary.py -x -q
"""
