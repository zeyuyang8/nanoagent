...
Each intermediate RUNNING :class:`~nanoagent.harness.core.agent.AgentResult` emitted to the ``on_step``
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/core/test_agent_on_step_snapshot.py -x -q
"""
...
class _ScriptedModel:
    """Scripted :class:`~nanoagent.harness.core.agent.ChatModel`: ``tool_steps`` single-``noop`` turns then
    a final ``"DONE"`` answer.
