...
final reply, so :meth:`~nanoagent.harness.core.agent.Agent.run` surfaces a best-effort answer via
:func:`~nanoagent.harness.core.agent.last_assistant_text` — the last assistant message that carried content.
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/core/test_agent_limit_answer_text.py -x -q
"""
...
class _ProgressModel:
    """A scripted :class:`~nanoagent.harness.core.agent.ChatModel` that carries content but NEVER answers.
