"""Pin Agent.run's tool-arg-malformation retry (:mod:`nanoagent.harness.core.agent`).
...
What it consumes: :class:`nanoagent.harness.core.agent.Agent`, :class:`~nanoagent.harness.core.agent.Reply`,
:class:`~nanoagent.harness.core.agent.ToolCall`, :class:`~nanoagent.harness.core.agent.StopReason` and one
:class:`~nanoagent.harness.core.tool.Tool` subclass — all in-process. No model server, network, or GPU.

Run (from the repo root)::

    python3 -m pytest tests/harness/core/test_agent_tool_arg_retry.py -x -q
"""
...
from nanoagent.harness.core.agent import Agent, Reply, StopReason, ToolCall, _MAX_TOOL_ARG_RETRIES
