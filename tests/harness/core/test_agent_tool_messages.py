...
:attr:`~nanoagent.harness.core.agent.AgentResult.messages` — exactly one per call, in input order, each keyed
...
:attr:`~nanoagent.harness.core.agent.AgentResult.tool_calls` side; this asserts only on ``result.messages``.
...
Run (from the repo root)::
    ...
    python3 -m pytest tests/harness/core/test_agent_tool_messages.py -q
"""
