...
Four subpackages, and the dependency arrows only ever point left:
  ...
  * :mod:`nanoagent.core` — the loop and what it is made of: ``agent``, ``tool``, ``model``,
    plus the seams ``hooks`` / ``events`` / ``workspace``. Imports nothing from the other three.
...
:class:`~nanoagent.core.model.Model` is a thin adapter over :mod:`leaninfer`, which resolves
``ModelConfig.backend`` against its own built-ins and then against the plugin directories in
``$LEANINFER_PLUGINS`` — so an SGLang server, an OpenAI-compatible gateway, or a private
transport dropped in as a plugin file all work with no change here.
