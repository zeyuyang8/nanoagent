...
Isolation gap: this mirrors the local-subprocess model of :class:`~nanoagent.harness.core.tool.Bash` and is
...
:class:`nanoagent.harness.core.tool.Tool` — not nanoagent.inference, not the search/scoring/training packages
...
from nanoagent.harness.core.tool import Tool, communicate_or_kill


class CodeExecutionError(RuntimeError):
    """Sandboxed code exited non-zero; the message carries the captured stdout + stderr (capped).

    Raised by :meth:`CodeExec.run` so the base :meth:`~nanoagent.harness.core.tool.Tool.invoke` turns it
...
# hand every rollout the same /tmp dir. Mirrors how nanoagent.harness.run.log_capture isolates each task's log
...
class CodeExec(Tool):
    ...
    def run(self, code: str) -> str:
        ...
        partial printed results) and stderr, so :meth:`~nanoagent.harness.core.tool.Tool.invoke` feeds them
        back to the model. A
