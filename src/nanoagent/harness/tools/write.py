...
:func:`~nanoagent.harness.core.tool.get_tools` already means a new tool is a new ``.py`` plus a new ``.yaml``
...
:func:`~nanoagent.harness.run.build.build_prompt_and_tools` globs that directory at startup, so a tool
...
the boundary :class:`~nanoagent.harness.tools.bash.Bash` and :class:`~nanoagent.harness.tools.code.CodeExec` already
...
from nanoagent.harness.core.tool import get_tools, JsonSchema, Tool
...
class WriteTool(Tool):
    ...
    `code` is a complete Python module defining one subclass of `nanoagent.harness.core.tool.Tool`:
