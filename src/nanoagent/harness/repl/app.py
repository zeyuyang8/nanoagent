...
else comes from the table in :mod:`nanoagent.harness.repl.commands` — ``/h`` help · ``/tree`` repo overview ·
...
:meth:`nanoagent.harness.core.agent.Agent.run` — the same loop the batch/benchmark path runs — over a
...
from nanoagent.harness.repl.commands import BUILTINS, Command, DEFAULT_THEME, image_in, inline_image, prompt_commands
from nanoagent.harness.config import InteractiveConfig, load_config_args, ModelConfig, RunConfig
from nanoagent.harness.core.events import EventWriter
from nanoagent.harness.core.hooks import get_hooks, Hooks
from nanoagent.harness.core.model import Model
from nanoagent.harness.repl.tree import load as load_session, SESSION_SUFFIX, SessionTree
from nanoagent.harness.core.tool import build_tool_map, JsonSchema, Tool
...
# Name of the tool :class:`~nanoagent.harness.tools.bash.Bash` registers; used to look it up in human mode.
_BASH_TOOL = "bash"
...
class _Narrating:
    ...
    Sits between :class:`~nanoagent.harness.core.agent.Agent` and the real model so everything the REPL
...
class InteractiveSession:
    ...
    :class:`~nanoagent.harness.config.AgentConfig` documents.
    ...
    # What a slash-command handler is allowed to reach for — see :mod:`nanoagent.harness.repl.commands`.
    @property
