...
(:mod:`nanoagent.harness.repl.app`) and ``browse`` (:mod:`nanoagent.harness.repl.browser`). The sibling commands are
...
:func:`~nanoagent.harness.core.tool.get_tools`), reproducing mini-swe-agent's behaviour against a local
...
:func:`~nanoagent.harness.config.load_config_args`), later tokens winning.
...
This module is the front end only: the batch driver itself is :mod:`nanoagent.harness.run.batch` and its
live display :mod:`nanoagent.harness.run.progress`.
...
(the conventional CWD, which every ``code:``/``*_cfg=`` path resolves against)::
  ...
  # chat — interactive session (see :mod:`nanoagent.harness.repl.app`); browse — trajectory viewer TUI:
...
from nanoagent.harness.config import (
    BatchConfig,
    load_config_args,
    peek,
    RunConfig,
)
from nanoagent.harness.run import log_capture
from nanoagent.harness.run.batch import filter_tasks, load_tasks, run_batch
from nanoagent.harness.run.build import build_agent
from nanoagent.harness.run.progress import BatchProgress, format_tally
...
def _run_single(argv: list[str]) -> int:
    ...
    from nanoagent.harness.repl.app import run_and_save
