...
paths; see :func:`~nanoagent.harness.core.tool.get_tools`) and ``task`` / ``output`` for what to run and
...
# names here, so `from nanoagent.harness.config import load_config` keeps working.
...
class ModelConfig:
    ...
    :meth:`~nanoagent.harness.core.model.Model.from_config`): the OpenAI SDK against ``base_url`` (an SGLang
    ...
    it, by the same names, so :meth:`~nanoagent.harness.core.model.Model.from_config` translates by field-name
...
class AgentConfig:
    ...
    # keeping the system prompt and the most recent exchange (see nanoagent.harness.core.agent.compact_messages).
    ...
    # :func:`~nanoagent.harness.core.hooks.get_hooks`). They can inject a reminder before a model turn or
    ...
    # Directory of skills — `<skills_dir>/<name>/SKILL.md` (see :mod:`nanoagent.harness.tools.skill`). Their
    ...
    # or a log tailer to consume live (see :mod:`nanoagent.harness.core.events`). null = don't write one.
...
class RunConfig:
    ...
    # `code:` module; see :func:`~nanoagent.harness.core.tool.get_tools`. The list *is* the toolset — add or
    ...
    # at startup (see :mod:`nanoagent.harness.tools.write`). Distinct from `tools` because those are the
