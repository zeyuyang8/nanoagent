...
(:mod:`nanoagent.harness.core.agent`) advertises tools to the model as OpenAI function
...
class Tool:
    ...
    def bind(self, agent: Any) -> None:
        """Receive the :class:`~nanoagent.harness.core.agent.Agent` this tool was given to.

        No-op by default, called once from ``Agent.__init__`` (and from
        :meth:`~nanoagent.harness.core.agent.Agent.add_tool`). The only way a tool can reach back into
        its agent — :mod:`nanoagent.harness.tools.write` needs it to register the tools it writes.
        """
...
def build_tool_map(tools: Sequence[Tool]) -> dict[str, Tool]:
    ...
    :class:`~nanoagent.harness.core.agent.Agent` and :class:`~nanoagent.harness.repl.app.InteractiveSession` build
...
def get_tools(yaml_paths: Iterable[str | Path]) -> list[Tool]:
    ...
    Returns a flat ``list[Tool]`` ready to hand to :class:`~nanoagent.harness.core.agent.Agent`'s
    ``tools``. Each YAML names the module that implements it (``code: src/nanoagent/harness/tools/bash.py``); that
    ...
    *defined* in it (classes merely imported into the module are ignored, and so is any
    ...
    ``code:`` may instead name an importable module (``code: nanoagent.harness.tools.bash``). That is
...
def load_module(path: Path) -> ModuleType:
    ...
    :mod:`nanoagent.harness.core.hooks` loads hook modules the same way.
    """
