...
:meth:`Tool.run <nanoagent.harness.core.tool.Tool.run>`, because concurrency here is already per-task: the
batch drivers dispatch every rollout as its own asyncio task under ``asyncio.gather``, and each
task gets its own copy of the context. So one shared :class:`~nanoagent.harness.core.agent.Agent` — the whole
