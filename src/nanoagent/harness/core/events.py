...
Everything is derived from the two callbacks :meth:`Agent.run <nanoagent.harness.core.agent.Agent.run>` already
...
"which rows have I emitted" counter is per-rollout — the same reason :class:`~nanoagent.harness.core.hooks.Hooks`
hands out a :class:`~nanoagent.harness.core.hooks.RunHooks`. Writes are plain ``write`` calls with no ``await``
...
class RunEvents:
    """One rollout's events. Created by :meth:`EventWriter.begin`, never directly."""

    def __init__(self, writer: EventWriter, label: str | None) -> None:
        self._writer = writer
        self._label = label
        self._emitted_tools = 0

    def on_delta(self, kind: str, text: str) -> None:
        self._writer.emit(self._label, type="delta", kind=kind, text=text)

    def on_step(self, result: AgentResult) -> None:
        from nanoagent.harness.core.agent import StopReason

        for row in result.tool_calls[self._emitted_tools :]:
            self._writer.emit(self._label, type="tool", **row)
