...
:class:`Model` is the single public backend (a :class:`~nanoagent.harness.core.agent.ChatModel`): the
...
loop side reaches into it. :class:`Model` just:
  ...
  1. translates the loop's :class:`~nanoagent.harness.config.ModelConfig` into a
  ...
  3. maps each :class:`~nanoagent.inference.types.Response` onto the loop's
     :class:`~nanoagent.harness.core.agent.Reply` (text + tool calls + usage + cost + reasoning).
...
from nanoagent.harness.core.agent import Reply, ToolCall
...
class Model:
    ...
    def from_config(cls, cfg: ModelConfig) -> Model:
        ...
        :class:`~nanoagent.harness.config.ModelConfig`'s fields are a strict subset of
