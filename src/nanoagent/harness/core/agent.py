...
This is the ONLY agent loop in the package. The batch driver (:mod:`nanoagent.harness.run.batch`,
a benchmark runner) calls :meth:`Agent.run` directly; the human-in-the-loop REPL
(:mod:`nanoagent.harness.repl.app`) drives the same method through its optional hooks
...
``hooks`` (:mod:`nanoagent.harness.core.hooks`) can inject a reminder before a model call, short-circuit a
tool call, or observe one; ``events`` (:mod:`nanoagent.harness.core.events`) mirrors the run to an NDJSON
stream. Both are split per-run inside :meth:`run` — a :class:`~nanoagent.harness.core.hooks.RunHooks` and a
:class:`~nanoagent.harness.core.events.RunEvents` — because the :class:`Agent` itself is shared.
...
Clean-room: depends only on :mod:`nanoagent.harness.core.tool` and the ``ChatModel`` duck-type below —
no ``openai`` import (the concrete backend lives in :mod:`nanoagent.harness.core.model`, which adapts
...
from nanoagent.harness.core.hooks import Hooks, RunHooks
from nanoagent.harness.core.tool import build_tool_map, Tool
...
class Reply:
    ...
    The model backend (:mod:`nanoagent.harness.core.model`) maps an inference ``Response`` into this; the
...
class StreamingChatModel(ChatModel, Protocol):
    ...
    like that — :class:`~nanoagent.harness.repl.app._Narrating` exists to print the stream, and calls
...
class Agent:
    def __init__(
        ...
        *,
        ...
        ``hooks`` (see :mod:`nanoagent.harness.core.hooks`) lets a config steer the loop — inject a reminder,
        refuse a tool call — without editing it. ``events`` (see :mod:`nanoagent.harness.core.events`) is the
    ...
    def add_tool(self, tool: Tool) -> None:
        ...
        Rebuilds the dispatch map through :func:`~nanoagent.harness.core.tool.build_tool_map` so a name
