...
Inputs:
  - ``path`` (a ``*.traj.json`` file, or a directory searched recursively for them,
    produced by :mod:`nanoagent.run.cli`).
...
from nanoagent.config import BrowseConfig, load_config_args
...
def render_message(message: dict[str, Any]) -> str:
    ...
    annotation (see :func:`nanoagent.run.trajectory._annotate_messages`).
    """
