...
(:mod:`nanoagent.repl.app`) and ``browse`` (:mod:`nanoagent.repl.browser`). The sibling commands are
...
:func:`~nanoagent.core.tool.get_tools`), reproducing mini-swe-agent's behaviour against a local
...
:func:`~nanoagent.config.load_config_args`), later tokens winning.
...
This module is the front end only: the batch driver itself is :mod:`nanoagent.run.batch` and its
live display :mod:`nanoagent.run.progress`.
...
(the conventional CWD, which every ``code:``/``*_cfg=`` path resolves against)::
  ...
  # chat — interactive session (see :mod:`nanoagent.repl.app`); browse — trajectory viewer TUI:
...
from nanoagent.config import (
    BatchConfig,
    load_config_args,
    peek,
    RunConfig,
)
from nanoagent.run import log_capture
from nanoagent.run.batch import filter_tasks, load_tasks, run_batch
from nanoagent.run.build import build_agent
from nanoagent.run.progress import BatchProgress, format_tally
...
def _run_single(argv: list[str]) -> int:
    ...
    from nanoagent.repl.app import run_and_save
...
_DELEGATES = {
    "chat": "nanoagent.repl.app",
    "browse": "nanoagent.repl.browser",
    # The one command that takes argparse flags rather than config tokens; it is also installed
    # as its own console script, `mgen` (see nanoagent.run.mgen for why it is the exception).
    "mgen": "nanoagent.run.mgen",
}
