...
from nanoagent.config import InteractiveConfig, load_config_args
from nanoagent.core.tool import PACKAGED_CONFIGS
from nanoagent.run.trajectory import TRAJECTORY_SUFFIX
...
def _traj_root() -> Path:
    # Deferred: nanoagent.repl.app builds a Model at import, which is the only openai import on
    # this path — `mgen --help` and `--dump-config` must not pay for it.
    from nanoagent.repl.app import DEFAULT_TRAJ_DIR
...
def run_print(cfg: InteractiveConfig, output_format: str) -> int:
    ...
    from nanoagent.repl.tree import load as load_session
...
def run_interactive(cfg: InteractiveConfig) -> int:
    from nanoagent.repl.app import ReplOptions, run_and_save
