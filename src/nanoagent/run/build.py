...
``RunConfig`` reuses nanoagent's :class:`~nanoagent.config.ModelConfig` /
:class:`~nanoagent.config.AgentConfig` verbatim, so one function serves both). A second copy of
...
if TYPE_CHECKING:
    ...
    from nanoagent.config import AgentConfig, ModelConfig
...
def build_prompt_and_tools(
    ...
    from nanoagent.core.tool import get_tools
...
def build_agent(
    ...
    from nanoagent.core.hooks import get_hooks
